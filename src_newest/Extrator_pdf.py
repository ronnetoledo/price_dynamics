...
def parse_blocks(text):
    # Divide em blocos por "Valor" (cabeçalho da coluna em cada tabela de resultados).
    # Compatível com tabelas que contenham ou não as linhas ASSET/TIMEFRAME.
    blocks = re.split(r'Valor\s+', text)
    results = []

    for block in blocks[1:]:
        if 'STEP' not in block:   # ignora blocos que não são tabelas de resultados
            continue
        data = {}
        try:
            # campos opcionais adicionados na versão atual
...
def compute_instability_min(csv_path, mode):
    ...
    # Agrupa por timeframe usando a coluna TIMEFRAME se disponível,
    # caso contrário divide o CSV em dois ao meio (comportamento legado).
    if 'TIMEFRAME' in df.columns and df['TIMEFRAME'].nunique() >= 2:
        ...
        # legado: sem coluna TIMEFRAME ou apenas 1 timeframe — usa S=S1=S2
        n = len(df) // 2
        if n == 0:
            # apenas 1 timeframe: duplica a série para preencher S1 e S2
            df1 = df.copy()
            df2 = df.copy()
        else:
            df1 = df.iloc[:n].copy()
    ...
    if mode == 'step':
        ...
        # sem pontos interiores válidos (n < 5) ou merged vazio
    ...
    # =========================
        ...
        # sem entradas válidas (lista vazia ou sem colunas A/B):
        # ocorre quando n=0 ou todos os valores são borda (n<=2)
