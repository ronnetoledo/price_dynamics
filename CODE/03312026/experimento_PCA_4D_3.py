...
# ================================================================
TIMEFRAME_FILES  = ['PETR4_M5.csv', 'PETR4_H1.csv']
...
def main_func(pdf,tf_file, n_value=5):
    ...
    prices = C  # close para reconstrução de preço
...
#if MULTI_STEP_TYPE == 'step':
def plot_fig_step(pdf,muti_data):
    ...
    ax_c.axvspan(10, 30, alpha=0.2, color='#4DAF4A', label=r'$\Delta\tau_{\rm opt}$')
    for sp in ['top', 'right']:
    ...
    fig2_prl.savefig(CSV_FILE.split('_')[0] + '_fig2_exponents.png', dpi=300,
                     bbox_inches='tight')
    print("Figura 2 salva.")
    pdf.savefig()
    plt.close()

def plot_fig_embed(pdf,multi_data):
# ── FIGURA 3 PRL: β vs EMBED_DIM ──────────────────────────────────────────
    ...
    ax3.axvspan(60, 80, alpha=0.18, color='#F0A500', label=r'$L_{\rm opt}$')
    ...
    fig3_prl.savefig(CSV_FILE.split('_')[0] + '_fig3_Lopt.png', dpi=300,
                     bbox_inches='tight')
